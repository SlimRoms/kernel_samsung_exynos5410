...
struct mdm_hsic_pm_data {
	...
	struct delayed_work req_resume_work;
...
void set_host_stat(const char *name, enum pwr_stat status)
{
	...
	if (pm_data->gpio_host_ready) {
		int dev_rdy = gpio_get_value(pm_data->gpio_device_ready);
		pr_info("mdm: host [%s], dev [%d]\n",
						status ? "ON" : "OFF", dev_rdy);
		/*
...
#define DEV_POWER_WAIT_SPIN	30
#define DEV_POWER_WAIT_MS	5
int wait_dev_pwr_stat(const char *name, enum pwr_stat status)
{
	int spin = DEV_POWER_WAIT_SPIN;
	/* find pm device from list by name */
	struct mdm_hsic_pm_data *pm_data = get_pm_data_by_dev_name(name);
...
static void mdm_hsic_rpm_check(struct work_struct *work)
{
	...
	if (lpa_handling) {
		pr_info("mdm: rpm ignore, in lpa handling\n");
		return;
	}
	...
	if (pm_runtime_resume(dev) < 0)
		queue_delayed_work(pm_data->wq, &pm_data->req_resume_work,
							msecs_to_jiffies(20));
				...
				unsigned long event, void *ptr)
{
	...
	case PM_SUSPEND_PREPARE:
		...
		pr_info("mdm: notifier: block irq\n");
		return NOTIFY_OK;
	case PM_POST_SUSPEND:
		pm_data->block_request = false;
		pr_info("mdm: notifier: unblock irq\n");
...
static irqreturn_t mdm_hsic_irq_handler(int irq, void *data)
{
	...
	pr_notice_ratelimited("mdm: irq[%s]%s\n", irq_level ? "R" : "F",
					pm_data->block_request ? ": B" : "");
	...
	/* do not call pm_request_resume in irq, move it to work struct */
#if 0
	if (pm_request_resume(&pm_data->udev->dev) < 0)
		pr_err("%s: unable to add pm work for rpm\n", __func__);
...
static int mdm_hsic_pm_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&pm_data->req_resume_work, mdm_hsic_rpm_check);
