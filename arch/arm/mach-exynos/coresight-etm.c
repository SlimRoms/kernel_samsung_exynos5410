...
#define ETM_POWERDOWN(cpu)				\
do {									\
	etm_writel(etm, cpu, POWERDOWN_MAGIC, ETMOSLAR); \
	mb();								\
} while (0)

#define ETM_POWERON(cpu)						\
do {									\
	mb();								\
	etm_writel(etm, cpu, 0x0, ETMOSLAR);			\
} while (0)
...
int etm_enable(int pm_enable)
{
	...
	etb_enable();
	funnel_enable(0, 0xFF);
	funnel_enable(5, 0xFF);
	funnel_enable(6, 0xFF);
