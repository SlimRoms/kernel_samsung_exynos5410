/* linux/drivers/video/backlight/s6e8fa0_I_mipi_lcd.c
 *
...
static int init_elvss_table(struct lcd_info *lcd, u8 *elvss_data)
{
	...
	for (k = 0; k < 2; k++) {
		...
		for (i = 0; i < ELVSS_STATUS_MAX; i++) {
			...
			lcd->elvss_table[k][i][1] = 0x2C;
			lcd->elvss_table[k][i][2] = elvss_data[1];
...
static int init_hbm_parameter(struct lcd_info *lcd, const u8 *mtp_data, const u8 *hbm_data)
{
	...
	lcd->elvss_table[0][ELVSS_STATUS_HBM][2] = 0x01;
	lcd->elvss_table[1][ELVSS_STATUS_HBM][2] = 0x01;
...
struct mipi_dsim_lcd_driver s6e8fa0_I_mipi_lcd_driver = {
	.probe		= s6e8fa0_probe,
...
static int s6e8fa0_init(void)
{
	return 0;
#if 0
	s5p_mipi_dsi_register_lcd_driver(&s6e8fa0_I_mipi_lcd_driver);
